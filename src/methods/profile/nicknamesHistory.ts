...
import { PROFILE_NICKNAMES_HISTORY as endpoint } from "../../endpoints"

import type { IBaseApiParams } from "../../types"
import type {
  IProfileNicknames,
  IProfileNicknamesResponse
...
export async function profileNicknames(
  params: IProfileNicknamesParams
): Promise<IProfileNicknamesResult> {
  const response = await call<IProfileNicknamesResponse>({
    path: endpoint
      .replace("{id}", params.id as string)
      .replace("{page}", params.page as string),
    token: params.token,
    ...params.request
  })
